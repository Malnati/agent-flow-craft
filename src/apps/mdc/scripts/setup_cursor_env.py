...
Instala o agente MCP no Cursor IDE para permitir os testes e integração.
"""
import os
import subprocess
import logging
...
logger = logging.getLogger('mcp_test_setup')

def main():
    """Configura o ambiente para testes do MCP no Cursor IDE."""
    ...
    try:
        ...
        # Localizar arquivo do agent.py na nova estrutura
        project_root = Path(__file__).resolve().parent.parent.parent.parent.parent
        agent_path = project_root / "src" / "apps" / "mdc" / "agent.py"
        
        if not agent_path.exists():
            logger.error(f"Arquivo MCP Agent não encontrado em: {agent_path}")
            logger.info("Buscando arquivo em localização alternativa...")
            
            # Verificar localização legada
            legacy_path = project_root / "src" / "scripts" / "mcp_agent.py"
            
            if legacy_path.exists():
                logger.info(f"MCP Agent encontrado na localização legada: {legacy_path}")
        ...
        subprocess.run(
            ["cp", str(agent_path), mcp_agent_path],
            check=True
        )
        subprocess.run(
